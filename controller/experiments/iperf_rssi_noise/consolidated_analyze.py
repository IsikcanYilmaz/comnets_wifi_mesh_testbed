...
def plotLossVsSeconds(loss, seconds, tx="", rx="", figtitle=""):
    ...
    ax.set_xlabel("Loss", fontsize=fontsize)
    ax.set_ylabel("Seconds", fontsize=fontsize)
...
def plotLossVsHops(loss, hops, tx="", rx="", figtitle=""):
    ...
    ax.set_xlabel("Loss", fontsize=fontsize)
    ax.set_ylabel("Hops", fontsize=fontsize)
...
def plotBitrateSentReceived(sent, received, tx="", rx="", figtitle="", averageWindow=50):
    ...
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel("Bitrate", fontsize=fontsize)
...
def plotBitrateAndDrops(bitrate, loss, tx="", rx="", figtitle="", averageWindow=50):
    ...
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel("Loss & Bitrate, normalized", fontsize=fontsize)
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
...
def plotAllTxPackets(allTxPackets, figtitle="", averageWindow=50):
    ...
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel("All Tx Packets", fontsize=fontsize)
    # ax.set_yscale('log')
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
...
def plotSumPerStationTxPackets(sumPerStationTxPackets, figtitle="", averageWindow=50):
    ...
    for me in sumPerStationTxPackets:
        ...
        ax.plot([i for i in range(0, len(data))], data, c=machines[me]["color"], label=me)
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel("Sum Per Station Tx Packets", fontsize=fontsize)
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
    ...
    for me in sumPerStationTxPackets:
        ...
        if len(data) == 0:
            continue
        ax.scatter([i for i in range(0, len(data))], data, c=machines[me]["color"], label=me, s=5)
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel("Sum Per Station Tx Packets", fontsize=fontsize)
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
...
def plotSnr(snr, figtitle="", averageWindow=50, loss=[]):
    ...
    plt.suptitle(f"Signal to Noise Ratio")
    for i, me in enumerate(snr):
        for peer in snr[me]:
            ...
            axs[i].scatter([i for i in range(0, len(data))], data, c=machines[peer]["color"], label=peer, s=2)
            axs[i].set_ylabel(f"{me} SNR (dBm)", color=machines[me]['color'], weight='bold', fontsize=14)
            if (i == 4):
                axs[i].set_xlabel("Trials", fontsize=fontsize)
            if (len(loss) != 0):
    ...
    labels = []
    # for i in axs:
    #     if (i.get_legend_handles_labels()[0] == []): # we may be dealing with an empty ax due to an experiment not having this node
    #         continue
    #     h = i.get_legend_handles_labels()[0][0]
    #     l = i.get_legend_handles_labels()[1][0]
    #     handles.insert(0, h)
    #     labels.insert(0, l)

    # BELOW BE HACKS
    [handles.append(axs[0].get_legend_handles_labels()[0][i]) for i in range(0, 4)]
    [labels.append(axs[0].get_legend_handles_labels()[1][i]) for i in range(0, 4)]
    [handles.append(axs[1].get_legend_handles_labels()[0][i]) for i in range(0, 4)]
    [labels.append(axs[1].get_legend_handles_labels()[1][i]) for i in range(0, 4)]

    # handles = list(set(handles))
    # labels = list(set(labels))
    handles.insert(0, handles[4]) # oof so hacky goddam
    labels.insert(0, labels[4])
    handles = handles[0:5]
    labels = labels[0:5]

    # pdb.set_trace()
    fig.legend(handles, labels, loc='upper right', markerscale=5)
    plt.tight_layout()
    ...
    for i, me in enumerate(snr):
        for peer in snr[me]:
            ...
            axs[i].set_ylabel(f"{me} SNR (dBm)", color=machines[me]['color'], weight='bold', fontsize=fontsize)
            if (i == 4):
                axs[i].set_xlabel("Trial", fontsize=fontsize)
            if (len(loss) != 0):
...
def plotBatmanMgmtTx(mgmtTx, figtitle="", averageWindow=50):
    ...
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel("Mgmt Tx Packets", fontsize=fontsize)
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
...
def plotBatmanMgmtRx(mgmtRx, figtitle="", averageWindow=50):
    ...
    for me in mgmtRx:
        ...
        ax.plot([i for i in range(0, len(data))], [i - data[0] for i in data], c=machines[me]["color"], label=me)
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel("Mgmt Rx Packets", fontsize=fontsize)
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
    ...
    for me in mgmtRx:
        ...
        ax.plot([i for i in range(0, len(data))], data, c=machines[me]["color"], label=me)
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel(f"Mgmt Rx Packets", fontsize=fontsize)
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
...
def plotBatmanMgmtTxBytes(mgmtTxBytes, figtitle="", averageWindow=50):
    ...
    for me in mgmtTxBytes:
        ...
        ax.plot([i for i in range(0, len(data))], [i - data[0] for i in data], c=machines[me]["color"], label=me)
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel("Mgmt Tx Bytes", fontsize=fontsize)
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
    ...
    for me in mgmtTxBytes:
        ...
        ax.plot([i for i in range(0, len(data))], data, c=machines[me]["color"], label=me)
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel("BATMAN Mgmt Tx Bytes", fontsize=fontsize)
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
...
def plotBatmanMgmtRxBytes(mgmtRxBytes, figtitle="", averageWindow=50):
    ...
    for me in mgmtRxBytes:
        ...
        ax.plot([i for i in range(0, len(data))], [i - data[0] for i in data], c=machines[me]["color"], label=me)
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel("Mgmt Rx Bytes", fontsize=fontsize)
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
...
def plotBatmanForward(forward, figtitle="", averageWindow=50):
    ...
    for me in forward:
        ...
        ax.plot([i for i in range(0, len(data))], [i - data[0] for i in data], c=machines[me]["color"], label=me)
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel("Forwarded Packets", fontsize=fontsize)
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
    ...
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
    plt.title(f"{averageWindow} Average Forwarded Packets Per Iteration")
...
def plotBatmanForwardBytes(forwardBytes, figtitle="", averageWindow=50):
    ...
    for me in forwardBytes:
        ...
        ax.plot([i for i in range(0, len(data))], [i - data[0] for i in data], c=machines[me]["color"], label=me)
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel("Forwarded Bytes", fontsize=fontsize)
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
    ...
    for me in forwardBytes:
        ...
        ax.plot([i for i in range(0, len(data))], data, c=machines[me]["color"], label=me)
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel(f"Forwarded Bytes", fontsize=fontsize)
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
...
def plotBatmanTx(batmanTx, batmanForward=None, batmanMgmt=None, figtitle="", averageWindow=50):
    ...
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel("BATMAN Tx Packets", fontsize=fontsize)
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
...
def plotBatmanTxDiff(batmanTx, batmanMgmtTx, figtitle="", averageWindow=50):
    ...
    for me in batmanTx:
        ...
        ax.plot([i for i in range(0, len(data))], [i - data[0] for i in data], c=machines[me]["color"], label=me)
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel("BATMAN Tx Diff", fontsize=fontsize)
    handles, labels = ax.get_legend_handles_labels()
    fig.legend(handles, labels, loc='upper right')
...
def singleHopCorrel(snr, loss, figtitle="", averageWindow=50):
    ...
    for i, me in enumerate(snr):
        for peer in snr[me]:
            ...
            axs.set_ylabel(f"{me} SNR (dBm)", color=machines[me]['color'], weight='bold', fontsize=fontsize)

            # pdb.set_trace()
            axs.scatter([i for i in range(0, len(data))], loss, label="loss percent", c="black", s=5)
            if (i == 4):
                axs.set_xlabel("Trial", fontsize=fontsize)
            if (len(loss) != 0):
                correl = np.corrcoef(data, loss)
                print(f"Correl between SNR of {me}-{peer} and loss: {correl[0][1]}")
                # print(correl)
    ...
    ax.set_xlabel("Trials", fontsize=fontsize)
    ax.set_ylabel("Correl", fontsize=fontsize)
    plt.show()
...
def parseConsolidatedFiles(dirname, start=None, end=None, averageWindow=500):
    ...
    print(f"Total bytes received percent {100 * totalBytesReceived / totalBytesSent}")
...
def parseConfigFile(dirname):
    global config
    f = open(f"{dirname}/.config")
    raw = f.read()
    f.close()
...
def main():
    ...
    print(args)
    plt.rcParams["figure.figsize"] = (15, 12) # Figure size
